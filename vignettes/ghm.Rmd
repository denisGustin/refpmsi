...
refpmsi::refpmsi("ghm_monorum_uhcd_public") %>% dplyr::glimpse()
...
```{r var ghm_monorum_uhcd_prive, echo = FALSE}

refpmsi::refpmsi("ghm_monorum_uhcd_prive") %>% dplyr::glimpse()
...
[Regroupements des GHM 2021](https://www.atih.sante.fr/sites/default/files/public/content/4071/regroupement_ghm_v2021.xlsx) (ATIH)  
...
[Regroupements des racines de GHM 2021](https://www.atih.sante.fr/sites/default/files/public/content/4071/regroupement_racinesghm_v2021.xlsx) (ATIH) 
...
[OVALIDE MCO - Tables de référence 2021](https://www.atih.sante.fr/sites/default/files/public/content/4082/ovalide_0.zip) (ATIH) 
...
[Données du volume 2 du manuel des GHS 2021 - format csv](https://www.atih.sante.fr/plateformes-de-transmission-et-logiciels/logiciels-espace-de-telechargement/id_lot/3111) (ATIH)  

## Exemples

### Chargement du référentiel des GHS Public des années 2020 et 2021  

```{r ghs_chargement_affichage, eval = FALSE}
# Chargement du référentiel des GHS-GHM Public des années 2020 et 2021
ghs_2020_2021 <- refpmsi::refpmsi("ghs_public",2020:2021)
ghs_2020_2021
```

```{r ghs_chargement, echo = FALSE}
ghs_2020_2021 <- refpmsi::refpmsi("ghs_public",2020:2021)
ghs_2020_2021
```
...
```{r ghm_association_libelle_affichage, eval = FALSE}
...
```{r ghm_association_libelle, echo = FALSE}
ghm_2019_2020 <- refpmsi::refpmsi("ghs_public",2019:2020)
...
```{r ghm_dms_affichage, eval = FALSE}
...
```{r ghm_dms_2.5_affichage, eval = FALSE}
...
```{r case_mix_da_affichage, eval = FALSE}
case_mix_da <- jeu_ghm %>% 
...
```{r ghm_gradation_affichage, eval = FALSE}
...
```{r ghs_intermediaire_affichage, eval = FALSE}
...
```{r mco_fg_listes_chargement_affichage, eval = FALSE}
...
```{r mco_fg_listes_chargement, echo = FALSE}
mco_fg_listes <- refpmsi::refpmsi("mco_fg_listes",2021)
mco_fg_listes
```
...
```{r d_0401, echo = FALSE}
liste_D_0401 <- mco_fg_listes %>% 
  dplyr::select(-c(tidyselect::contains("ccam"),"mco_fg_liste_type","annee_pmsi")) %>% 
  dplyr::filter(mco_fg_liste_code == "D-0401")
liste_D_0401
```
...
```{r diag_cmd_affichage, eval = FALSE}
# on a chargé le référentiel mco_fg_listes dans la variable mco_fg_listes
...
```{r jeu_dp_ghm_liste_D_affichage,eval = FALSE}
...
```{r jeu_dp_ghm_liste_D, echo = FALSE}
jeu_dp_ghm_liste_D <- jeu_dp_ghm %>% 
  dplyr::mutate(cmd_rss = stringr::str_sub(ghm_rss,1,2)) %>% 
  dplyr::left_join(diag_cmd %>% tidyr::unnest(diag_entree),
                            by = c("dp_rss" = "mco_fg_code",
