...
Ce référentiel annualisé au sens PMSI (2017 à 2021) comprend 10 variables et 209 626 lignes.  

Son chargement complet en local et/ou sa visualisation dans R Studio peuvent être longs. Il est recommandé de ne charger que la ou les années utile(s).  
...
```
### Sources  
[Nomenclature CIM-10 2021 : Fichiers nomenclature CIM10 pour les champs MCO,HAD, SSR et PSY](https://www.atih.sante.fr/plateformes-de-transmission-et-logiciels/logiciels-espace-de-telechargement/telecharger/gratuit/14692/456) (ATIH)  
...
[CIM-10 FR à usage PMSI 2017 - version du 30 mai 2017](http://social-sante.gouv.fr/fichiers/bos/2017/sts_20170009_0001_p000.pdf) (ATIH)  
...
Le référentiel n'est pas annualisé. Il est calé sur la dernière version CIM-10 à usage PMSI (2021)  
...
[Guide de lecture OVALIDE MCO 2021](https://sap.atih.sante.fr/epmsi/doc/guides/lecture/guide_lecture_OVALIDE_SSR_DGF_2021.pdf) (ATIH)  
...
```{r cim_chargement_one_year, eval = FALSE}
# Chargement de la CIM-10 de l'année 2021
cim_2021 <- refpmsi::refpmsi("cim",2021)
head(cim_2021)
```

```{r cim_chargement_one_year_affichage, echo = FALSE}
cim_2021 <- refpmsi::refpmsi("cim",2021)
head(cim_2021)
```
...
jeu_dp <- tibble::tibble(no_rss = seq(1:12),
                         dp_rss = c("I501","Z491","H251","Z380",
                                    "S7200","S0600","R074",
                                    "J441","K358","R53+0","S7200","I501"),
                         annee_rss = sample(c("2019","2020"),12,replace = TRUE))
jeu_dp
```
...
jeu_dp_libelle <- jeu_dp %>% 
  dplyr::left_join(cim_2019_2020 %>% dplyr::select(cim_code,cim_lib,annee_pmsi),
...
das_interdit_dp_dr <- jeu_das %>% 
  ...
  dplyr::left_join(cim_2019_2020 %>% dplyr::select(cim_code,cim_lib,cim_tmco,annee_pmsi),
